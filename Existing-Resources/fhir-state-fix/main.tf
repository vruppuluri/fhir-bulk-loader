...
locals {
  # Use existing_suffix when provided (re-applying to already-deployed infra).
  # Falls back to the random_string resource on a fresh deployment.
  suffix    = var.existing_suffix != "" ? var.existing_suffix : random_string.suffix.result
